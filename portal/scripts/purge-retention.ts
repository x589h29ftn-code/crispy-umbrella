...
console.log(`Bewaartermijn verstreken: ${status.blobsDue + status.dossiersDue} dossier(s)`)
console.log(`Eerstvolgende verloopt   : ${status.next ? status.next.toISOString().slice(0, 10) : '-'}`)

if (status.blobsDue + status.dossiersDue === 0) {
  console.log('\nNiets te doen.')
} else {
  const res = await purgeExpiredDossiers({ dryRun: !apply, limit: 1000 })
  if (apply) {
    console.log(`\nOpgeruimd: ${res.blobsGewist + res.dossiersVerwijderd} dossier(s), ${res.documents} document(en), ${res.auditEvents} auditregel(s).`)
    if (res.skipped.length) console.error('Overgeslagen:', res.skipped)
  } else {
    console.log(`\nZou opruimen: ${res.blobsGewist + res.dossiersVerwijderd} dossier(s), ${res.documents} document(en), ${res.auditEvents} auditregel(s).`)
