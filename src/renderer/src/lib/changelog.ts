...
export const APP_CHANGELOG: { version: string; items: string[] }[] = [
  {
    ...
    items: [
      ...
      'Nieuw: opslaan als PDF/A (archief), aan te zetten bij Voorkeuren of in het eigenschappenvenster. Het bestand wordt PDF/A-2b: alle tekst die PDF Studio zelf toevoegt (tekstvakken, watermerk, paginanummers, stempels, veldlabels) wordt met een ingebed lettertype geschreven — ook als je Helvetica, Times of Courier kiest, want die zitten niet in een PDF — er komt een sRGB-kleurprofiel in het bestand als OutputIntent, de documentgegevens worden naast de gewone eigenschappen ook als XMP-metadata opgenomen, en het document krijgt een vaste identificatie in de trailer',
      'Het sRGB-profiel wordt door de app zelf opgebouwd (6,6 kB, in de PDF gecomprimeerd 2,3 kB) met de primaries en het witpunt van sRGB IEC61966-2.1 en de sRGB-toonkromme als tabel van 1024 punten. Gecontroleerd met een echte kleurmotor (LittleCMS): die leest het profiel als "sRGB IEC61966-2.1" en vijf testkleuren komen na omzetten heen en terug precies hetzelfde uit',
      'Eerlijk over de grens van PDF/A: lettertypen die in het geopende bestand zelf niet zijn ingebed kan geen enkel programma alsnog toevoegen — de letters zitten simpelweg niet in het bestand. Bij het opslaan noemt PDF Studio ze bij naam ("de lettertypen Helvetica, Helvetica-Bold zitten niet in het bronbestand ingebed") en zegt erbij dat het bestand dan niet gegarandeerd geldig PDF/A is. De export is niet met een validator zoals veraPDF getoetst. Een PDF/A-bestand kan ook geen wachtwoord hebben; staat er een wachtwoord of een rechtenbeperking ingesteld, dan meldt de app dat die niet wordt toegepast',
      'Nieuw: bevestiging bij afsluiten. Zolang er bewerkingen zijn die nog niet zijn opgeslagen, vraagt PDF Studio bij het sluiten van het venster of je echt wilt afsluiten, met "Afsluiten" en "Annuleren" (Annuleren is voorgeselecteerd). Na opslaan of exporteren vervalt de vraag weer. Uit te zetten bij Voorkeuren → "Vragen voordat de app afsluit"',
      'Slim splitsen kan nu op vier manieren in plaats van alleen op bladwijzers: op bladwijzers, per hoofdstuk, per aantal pagina\'s en bij lege pagina\'s. Je kiest de manier en ziet meteen welke documenten eruit komen, met naam en paginabereik, voordat je splitst',
      'Splitsen per hoofdstuk herkent de koppen in de tekst zelf (lettergrootte, vet en de bladwijzers samen) en knipt alleen waar een kop bovenaan een pagina staat. Het hoofdstukniveau wordt bepaald op het hoogste niveau dat minstens twee keer bovenaan een pagina voorkomt, zodat de titel op het voorblad de indeling niet verpest. Getest op een rapport van 7 pagina\'s: vier documenten met de juiste namen ("Balans per 31 december 2025", "Winst-en-verliesrekening 2025", "Toelichting op de balans") en de juiste pagina-aantallen',
      'Splitsen bij lege pagina\'s gebruikt blanco scheidingsbladen als grens en laat die pagina\'s zelf weg — handig voor een stapel bonnen die je in één keer inscant. Getest op 7 pagina\'s met twee scheidingsbladen: drie documenten van 2, 2 en 1 pagina',
      'Nieuw gereedschap Invulveld (sneltoets I) om zelf een PDF-formulier te bouwen: tekstveld, meerdere regels, datum, bedrag, vinkje, keuzerondje, keuzelijst en handtekeningvak. Sleep een vak op de pagina of klik voor een veld op standaardformaat, en geef het een label, een groepsnaam (keuzerondjes met dezelfde groep sluiten elkaar uit), keuzes voor een keuzelijst en desgewenst "verplicht invullen"',
      'Bij het opslaan worden dit échte invulvelden in de PDF, dus de ontvanger vult ze in Acrobat, Edge of Chrome gewoon in — geen plaatje van een formulier. Gecontroleerd door het opgeslagen bestand weer te openen: drie tekstvelden (waarvan één met datumlengte en één rechts uitgelijnd voor een bedrag), een keuzelijst met de drie ingevulde keuzes, een vinkje en een keuzerondje, allemaal invulbaar. Het handtekeningvak blijft een getekend kader met een lijn om op te ondertekenen',
      'Velden die je zelf plaatst blijven invulbaar, ook als "Formulieren platslaan bij export" aan staat — die instelling gaat over de velden die al in het geopende bestand zaten. In PDF Studio zie je het veld als kader met het opschrift op dezelfde plek als in de opgeslagen PDF; invullen doe je in de PDF (of met het gereedschap Formulier als je het bestand opnieuw opent)',
      'Het sneltoetsen-overzicht laat nu ook de letters van het gereedschap in het leestabblad zien (V, M, P, S, K, T, B, I, F, R, C, E)'
    ]
  },
  {
    version: '1.13.0',
