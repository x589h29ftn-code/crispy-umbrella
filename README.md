...
- **Zoeken in alle documenten**: Ctrl+F (of de knop "Zoeken" in het zijmenu) opent een zoekpaneel dat door de tekst van álle geladen documenten zoekt, met fragmenten en trefferaantallen per pagina; klikken springt direct naar die pagina in het volledig scherm.
- **Tekstherkenning (OCR) voor scans**: het zoekpaneel meldt gescande pagina's zonder tekstlaag; één klik op "Tekst herkennen (OCR)" herkent ze (Nederlands, via tesseract.js in het main-proces — volledig offline, de taaldata zit in de installer). Daarna zijn de scans doorzoekbaar in de app én krijgt de export een onzichtbare tekstlaag op woordpositie, zodat de PDF ook in andere programma's selecteerbaar en doorzoekbaar is.
