...
- **Soepel bij grote documenten**: pagina-miniaturen én de pagina's in de bewerk-/leesweergave worden pas op hoge resolutie gerenderd wanneer ze (bijna) in beeld komen — niet meer allemaal tegelijk bij het openen. Afbeeldingen worden als blob buiten beeld gedecodeerd en tijdens (Ctrl-)zoomen wordt pas scherp opnieuw gerenderd zodra het zoomen tot rust komt. Zo blijft scrollen door documenten van honderden pagina's vloeiend in plaats van hakkerig.
...
- **Tabellen naar Excel (met opmaak)**: herkent rijen en kolommen uit de tekstposities en exporteert ze naar een `.xlsx`-bestand (één werkblad per pagina) met nette celranden, en getallen/bedragen/percentages/datums als échte waarden (niet als tekst) met automatische kolombreedtes. Overzichten met een kopregel (zoals jaar-op-jaar) krijgen een vetgedrukte, filterbare kopregel die in beeld blijft. Onder "Slimme documenten → Tabel → Excel". Werkt op de tekstlaag; voor scans eerst OCR draaien.
- **Exporteren naar bewerkbaar Word (.docx)**: onder "Slimme documenten → Tekst → Als Word" maak je een echt Word-document waarin de lettergroottes en koppen behouden blijven (koppen worden vet) en elke pagina op een nieuwe pagina begint — dus een bewerkbaar document in plaats van één blok platte tekst. (Voor complexe grafische lay-outs is dit een goede benadering, geen exacte 1-op-1-kopie.)
...
- **Werkbalk verbergen**: klik op het kruisje bovenin de zijbalk om de hele werkbalk te verbergen voor maximale documentruimte; linksboven verschijnt dan een duidelijke blauwe knop **"Werkbalk"** waarmee je hem weer tevoorschijn haalt.
...
- **Nauwkeurigere AVG-scan**: een BSN wordt herkend als 9 aaneengesloten cijfers (11-proef), of gegroepeerd (123 456 789) alléén wanneer het woord "BSN"/"sofinummer" op de regel staat — zo worden bedragen in een jaarrekening (met duizendtal-scheiding) niet meer per ongeluk zwart gelakt. Bedrijfsnamen met tussenwoorden ("Timmerfabriek De Houtmolen Makkum B.V.", "Van der Meer Bouw B.V.") en rechtsvormen met losse punten/spaties (B. V.) worden nu ook herkend.
